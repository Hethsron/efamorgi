...
public class ServiceDao implements Dao<Service> {
    ...
    private final Map<Long, Service> store = Collections.synchronizedMap(new TreeMap<Long, Service>());
    ...
    public void update(Service service, String[] params) {
        service.setName(Objects.requireNonNull(params[0], "Service name cannot be null"));
        service.setDescription(Objects.requireNonNull(params[1], "Service description cannot be null"));
        store.put(service.getId(), service);
    }

    @Override
    public void remove(Service service) {
        store.remove(service.getId());
    }
